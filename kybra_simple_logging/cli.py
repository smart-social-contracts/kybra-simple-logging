...
def parse_args():
    ...
    # Follow mode options
    parser.add_argument('--follow', action='store_true', help='Follow logs (poll every 5 seconds)')
    parser.add_argument('--interval', type=int, default=5, help='Polling interval in seconds for follow mode')
    
    # Network options
    network_group = parser.add_mutually_exclusive_group()
    network_group.add_argument('--network', help='Network URL (e.g., http://localhost:4943)')
...
def format_log(log_entry):
    ...
    try:
        timestamp = datetime.fromtimestamp(int(log_entry["timestamp"]) / 1e9).strftime('%Y-%m-%d %H:%M:%S')
